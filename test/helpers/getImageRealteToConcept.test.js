...
describe('getImageRealteToConcept testing', () => {
    ...
    it('Test error retrieving the webpage images source', () => {
        ...
        axios.get
            .mockImplementationOnce(() => {
                throw 'error.getImage'
            })

        return getImageRealteToConcept('good concept').then(result => {
            expect(result).toEqual('./assets/oops.jpg');
            expect(logger.error).toHaveBeenCalledWith('error.getImage');
        });;
    })
    ...
    it('Test fail in downloadImage axios.get', () => {
        jest.spyOn(global.Math, 'floor').mockReturnValueOnce(0);
        axios.get
            .mockImplementationOnce(() => {
                return { data: 'testdata' }
        ...
        return getImageRealteToConcept('concept').then(result => {
            expect(result).toEqual('./assets/oops.jpg');
            expect(logger.error).toHaveBeenCalledWith('error.downloadImage');
        });
    });
